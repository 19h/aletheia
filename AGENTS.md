...
- [ ] **M.1** Implement Additional Out-of-SSA Strategies (currently only basic Sreedhar-like approach)
  ...
  - [ ] M.1.1 Implement `MinimalVariableRenamer` with Lex-BFS graph coloring on the interference graph.
...
- [ ] **M.13** Implement the 5 Preprocessing Stages for Real (currently all empty stubs)
  ...
  - [ ] M.13.5 `RemoveStackCanaryStage`: detect `__stack_chk_fail` leaf nodes and remove the branch condition.
