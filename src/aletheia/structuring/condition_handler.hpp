...
public:
    ...
    logos::LogicCondition add_condition(Condition* condition) {
        if (!condition) {
            return logos::LogicCondition(ctx_.bool_val(true));
        }

        if (auto it = condition_to_symbol_.find(condition); it != condition_to_symbol_.end()) {
            return logos::LogicCondition(ctx_.bool_const(it->second.c_str()));
        }
        ...
        return logos::LogicCondition(ctx_.bool_const(symbol.c_str()));
    }
