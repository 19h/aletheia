...
void Lifter::populate_task_signature(DecompilerTask& task) {
    ...
    current_function_prefers_w_args_ = state.current_function_prefers_w_args;
    current_function_prefers_w_return_ = state.current_function_prefers_w_return;
}

void Lifter::populate_frame_layout(ida::Address function_address) {
    frame_layout_ = frontend::build_frame_layout(function_address);
}

std::optional<std::string> Lifter::resolve_frame_variable(std::int64_t frame_offset,
                                                           std::size_t access_size) const {
    return frontend::resolve_frame_variable(frame_layout_, frame_offset, access_size);
}
...
Variable* Lifter::resolve_sp_relative_slot(ida::Address insn_addr,
    ...
    return frontend::resolve_sp_relative_slot(arena_, frame_layout_, insn_addr, sp_adjust_bytes, access_size);
}
