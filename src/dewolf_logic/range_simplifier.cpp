...
static bool is_comparison(dewolf::OperationType op) {
    using OT = dewolf::OperationType;
    switch (op) {
...
static bool is_signed_comparison(dewolf::OperationType op) {
    using OT = dewolf::OperationType;
    switch (op) {
        case OT::eq: case OT::neq:
        case OT::lt: case OT::le: case OT::gt: case OT::ge:
...
static bool is_strict_comparison(dewolf::OperationType op) {
    using OT = dewolf::OperationType;
    return op == OT::lt || op == OT::gt || op == OT::lt_us || op == OT::gt_us;
}

/// Is the comparison a non-equal (<=, <, >, >=) relation?
static bool is_non_equal_relation(dewolf::OperationType op) {
    using OT = dewolf::OperationType;
...
static OrderInfo get_order_info(dewolf::OperationType op) {
    using OT = dewolf::OperationType;
    switch (op) {
...
void ExpressionValues::update_with(
    dewolf::OperationType op, std::int64_t const_val, bool const_is_lhs)
{
    using OT = dewolf::OperationType;
...
std::optional<BoundRelation> BoundRelation::from(dewolf::Expression* expr) {
    using OT = dewolf::OperationType;

    // Must be a Condition (binary comparison)
    auto* cond = dynamic_cast<dewolf::Condition*>(expr);
    if (!cond) {
        // Also accept a generic Operation with a comparison type and 2 operands
        auto* op = dynamic_cast<dewolf::Operation*>(expr);
        if (!op || op->operands().size() != 2 || !is_comparison(op->type()))
            return std::nullopt;
        // Use the Operation directly
        auto* lhs = op->operands()[0];
        auto* rhs = op->operands()[1];
        auto* lhs_const = dynamic_cast<dewolf::Constant*>(lhs);
        auto* rhs_const = dynamic_cast<dewolf::Constant*>(rhs);
...
dewolf::Expression* SingleRangeSimplifier::simplify(
    dewolf::Expression* expr, dewolf::DecompilerArena& arena)
{
    using OT = dewolf::OperationType;
    ...
    if (is_strict_comparison(ot)) {
        ...
        dewolf::Expression* smaller_operand = info.smaller_is_lhs ? lhs : rhs;
        dewolf::Expression* greater_operand = info.smaller_is_lhs ? rhs : lhs;

        auto* smaller_const = dynamic_cast<dewolf::Constant*>(smaller_operand);
        auto* greater_const = dynamic_cast<dewolf::Constant*>(greater_operand);
        ...
        if (unfulfillable) {
            ...
            return arena.create<dewolf::Constant>(0, 1);
        }
        ...
        if (diff == 1) {
            ...
            dewolf::Expression* var_operand = smaller_const ? greater_operand : smaller_operand;
            ...
            auto* bound_const = arena.create<dewolf::Constant>(
                static_cast<std::uint64_t>(var_bound), var_operand->size_bytes);
            return arena.create<dewolf::Condition>(OT::eq, var_operand, bound_const, 1);
        }
    }
    ...
    if (ot == OT::le || ot == OT::ge || ot == OT::le_us || ot == OT::ge_us) {
        ...
        if (smaller_const && static_cast<std::int64_t>(smaller_const->value()) == min_val) {
            return arena.create<dewolf::Constant>(1, 1); // always true
        }
        ...
        if (bounds_equal) {
            return arena.create<dewolf::Condition>(OT::eq, lhs, rhs, 1);
        }
    }
...
bool BitwiseAndRangeSimplifier::is_unfulfillable(
    const std::vector<dewolf::Expression*>& and_operands)
{
    ...
    auto find_or_create = [&](dewolf::Expression* var_expr, std::size_t bit_size) -> ExpressionValues& {
        for (auto& vi : var_infos) {
            // Match by pointer identity or by Variable name+ssa_version
            if (vi.var_expr == var_expr) return vi.values;
            auto* v1 = dynamic_cast<dewolf::Variable*>(vi.var_expr);
            auto* v2 = dynamic_cast<dewolf::Variable*>(var_expr);
...
dewolf::Expression* BitwiseAndRangeSimplifier::simplify(
    dewolf::Operation* op, dewolf::DecompilerArena& arena)
{
    if (!op || (op->type() != dewolf::OperationType::logical_and && op->type() != dewolf::OperationType::bit_and)) {
        return op;
    }
    ...
    std::erase_if(and_operands, [](dewolf::Expression* e) {
        auto* c = dynamic_cast<dewolf::Constant*>(e);
        return c && c->value() == 1;
    });
    ...
    for (auto* child : and_operands) {
        auto* c = dynamic_cast<dewolf::Constant*>(child);
        if (c && c->value() == 0) {
            return arena.create<dewolf::Constant>(0, 1);
        }
    }
    ...
    std::vector<dewolf::Expression*> remaining_operands;
    for (auto* operand : and_operands) {
        auto br = BoundRelation::from(operand);
        if (br) {
            std::size_t bit_size = br->variable_expr->size_bytes * 8;
    ...
    std::vector<dewolf::Expression*> new_operands;
    for (auto& vi : var_infos) {
        vi.values.simplify();
        ...
        if (vi.values.must_values().size() == 1) {
            ...
            auto* const_expr = arena.create<dewolf::Constant>(static_cast<std::uint64_t>(val), var_expr->size_bytes);
            new_operands.push_back(arena.create<dewolf::Condition>(dewolf::OperationType::eq, var_expr, const_expr, 1));
        } else {
            // Emit bounds constraints
            const auto& upper = vi.values.upper_bound();
            if (upper.signed_val) {
                auto* const_expr = arena.create<dewolf::Constant>(static_cast<std::uint64_t>(*upper.signed_val), var_expr->size_bytes);
                new_operands.push_back(arena.create<dewolf::Condition>(dewolf::OperationType::le, var_expr, const_expr, 1));
            }
            if (upper.unsigned_val) {
                auto* const_expr = arena.create<dewolf::Constant>(*upper.unsigned_val, var_expr->size_bytes);
                new_operands.push_back(arena.create<dewolf::Condition>(dewolf::OperationType::le_us, var_expr, const_expr, 1));
            }

            const auto& lower = vi.values.lower_bound();
            if (lower.signed_val) {
                auto* const_expr = arena.create<dewolf::Constant>(static_cast<std::uint64_t>(*lower.signed_val), var_expr->size_bytes);
                new_operands.push_back(arena.create<dewolf::Condition>(dewolf::OperationType::ge, var_expr, const_expr, 1));
            }
            if (lower.unsigned_val) {
                auto* const_expr = arena.create<dewolf::Constant>(*lower.unsigned_val, var_expr->size_bytes);
                new_operands.push_back(arena.create<dewolf::Condition>(dewolf::OperationType::ge_us, var_expr, const_expr, 1));
            }
                    ...
                    if (static_cast<std::uint64_t>(current_row.back()) + 1 == static_cast<std::uint64_t>(sorted_fvals[i])) {
                        ...
                        if (current_row.size() <= 2) {
                            for (auto c : current_row) {
                                auto* const_expr = arena.create<dewolf::Constant>(static_cast<std::uint64_t>(c), var_expr->size_bytes);
                                new_operands.push_back(arena.create<dewolf::Condition>(dewolf::OperationType::neq, var_expr, const_expr, 1));
                            }
                        } else {
                            // range exclusion using bitwise NOT of AND
                            auto* c1 = arena.create<dewolf::Constant>(static_cast<std::uint64_t>(current_row.front()), var_expr->size_bytes);
                            auto* lower_bound = arena.create<dewolf::Condition>(dewolf::OperationType::le_us, c1, var_expr, 1);
                            auto* c2 = arena.create<dewolf::Constant>(static_cast<std::uint64_t>(current_row.back()), var_expr->size_bytes);
                            auto* upper_bound = arena.create<dewolf::Condition>(dewolf::OperationType::le_us, var_expr, c2, 1);
                            auto* and_op = arena.create<dewolf::Operation>(dewolf::OperationType::bit_and, std::vector<dewolf::Expression*>{lower_bound, upper_bound}, 1);
                            new_operands.push_back(arena.create<dewolf::Operation>(dewolf::OperationType::logical_not, std::vector<dewolf::Expression*>{and_op}, 1));
                        }
    ...
    if (new_operands.size() == 0) {
        return arena.create<dewolf::Constant>(1, 1);
        ...
        if (structure_changed) {
            return arena.create<dewolf::Operation>(op->type(), new_operands, 1);
        }
        return op;
    }
}
...
dewolf::Expression* BitwiseOrRangeSimplifier::simplify(
    dewolf::Operation* op, dewolf::DecompilerArena& arena)
{
    if (!op || (op->type() != dewolf::OperationType::logical_or && op->type() != dewolf::OperationType::bit_or)) {
        return op;
    }
    ...
    std::vector<dewolf::Expression*> negated_operands;
    for (auto* child : op->operands()) {
        auto* negated_child = arena.create<dewolf::Operation>(dewolf::OperationType::logical_not, std::vector<dewolf::Expression*>{child}, 1);
        negated_operands.push_back(negated_child);
    }

    auto* new_and_op = arena.create<dewolf::Operation>(dewolf::OperationType::logical_and, negated_operands, 1);

    auto* simplified_and = BitwiseAndRangeSimplifier::simplify(new_and_op, arena);

    if (auto* c = dynamic_cast<dewolf::Constant*>(simplified_and)) {
        if (c->value() == 0) {
            // Not(False) == True
            return arena.create<dewolf::Constant>(1, 1);
        } else if (c->value() == 1) {
    ...
    // Negate it back
    return arena.create<dewolf::Operation>(dewolf::OperationType::logical_not, std::vector<dewolf::Expression*>{simplified_and}, 1);
}
...
bool RangeSimplifier::is_unfulfillable(dewolf::Expression* condition) {
    using OT = dewolf::OperationType;
    ...
    auto* op = dynamic_cast<dewolf::Operation*>(condition);
    if (op && op->type() == OT::logical_and) {
        // Collect all AND operands (potentially nested)
        std::vector<dewolf::Expression*> all_operands;
        std::function<void(dewolf::Expression*)> collect = [&](dewolf::Expression* e) {
            auto* inner = dynamic_cast<dewolf::Operation*>(e);
...
dewolf::Expression* RangeSimplifier::simplify(
    dewolf::Expression* condition, dewolf::DecompilerArena& arena)
{
    using OT = dewolf::OperationType;
    ...
    if (op && (op->type() == OT::logical_and || op->type() == OT::bit_and)) {
        ...
        std::erase_if(operands, [](dewolf::Expression* e) {
            auto* c = dynamic_cast<dewolf::Constant*>(e);
            return c && c->value() == 1;
        });
        ...
        if (operands.empty()) return arena.create<dewolf::Constant>(1, 1);
    ...
    if (op && (op->type() == OT::logical_or || op->type() == OT::bit_or)) {
        ...
        if (operands.size() == 1) return operands[0];
        if (operands.empty()) return arena.create<dewolf::Constant>(0, 1);
    }
